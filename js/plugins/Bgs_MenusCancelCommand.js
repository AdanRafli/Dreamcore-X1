 ...
 *
 * @help This plugin adds a cancel command to menus, including in shops, so the user will
 * never have to press the cancel button (Esc / right-click / two-finger-tap).
 * 
 ...
 * Update log:
 *   1.0: Initial release.
 *   1.1: Exit shop by buying / selling 0 items
 * 
...
(function (_) {
    ...
    Window_Status.prototype.refresh = function () {
        ...
        if (this._actor && _.IncludeInStatus == "true") {
            ...
            this.drawText(_.CancelCommandName, 6, lineHeight * 15);
        }
    };
    ...
    _.Window_ShopNumber_setup = Window_ShopNumber.prototype.setup;
    Window_ShopNumber.prototype.setup = function (item, max, price) {
        this._item = item;
        this._max = Math.floor(max);
        this._price = price;
        this._number = 0; // changed from 1 to 0
        this.placeButtons();
        this.updateButtonsVisiblity();
        this.refresh();
    };

    _.WindowWindow_ShopNumber_changeNumber = Window_ShopNumber.prototype.changeNumber;
    Window_ShopNumber.prototype.changeNumber = function (amount) {
        var lastNumber = this._number;
        this._number = (this._number + amount).clamp(0, this._max); // changed the clamp from 1 to 0
